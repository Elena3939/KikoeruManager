...
- 样式优先级：
  ...
  4. 前端能用lottie动画的地方考虑使用
  5. 所有图标必须要有动效以及鼠标放置变图标等交互动画
...
### 3.8 按钮动画规范 (Interaction & Animation)

- **核心原则**：所有交互元素（按钮、卡片）必须具备物理感、弹性感和明确的视觉反馈。
- **状态反馈规范**：
  - **Hover (悬停)**：
...
- 当前已覆盖：
  ...
  - 字幕过滤规则
- 当前工作台布局口径：
  - 顶部 `任务总览 / 筛选与配对 / 字幕文件树` 是主舞台切换，不要把阶段内容混在同一块里
  - 中间横向任务条只负责切换任务，不要重复塞大段详情
  - `筛选与配对` 顶部信息卡只放目录标题、meta chip 和任务操作按钮
  - `自动预配对 / 顺序点选配对 / 按当前列表预配对 / 一键应用同名` 只能留在配对结果区头部，不能挪到顶部目录卡
  - `任务总览` 主面板当前只展示选中任务执行日志，不再重复渲染一整张任务详情卡
- 当前任务状态口径：
  - `awaiting_manual_match` 虽然底层常还是 `pending`，但前端交互上已经算进入“筛选与配对”阶段
  - 这类任务允许“重新执行爬取字幕”，不要再按普通 `pending` 一刀切禁用
  - 扫描区显示“已入任务 N”时，任务总栏和任务队列也必须合并显示这些已绑定 `task_id` 但还没被状态轮询带回的任务
