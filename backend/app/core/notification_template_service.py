...
_DEFAULT_SUBJECT = {
    'completed': '[Prekikoeru] {domain_label}任务完成 — {title}',
    'failed': '[Prekikoeru] {domain_label}任务失败 — {title}',
    'waiting_manual': '[Prekikoeru] 等待人工处理 — {title}',
}
...
def render_email_for_outbox(payload: dict) -> tuple:
    ...
    try:
        ...
        if tpl and tpl.html_template:
            return _render_user_template(tpl, payload)
    except Exception:
        pass
    finally:
        db.close()
...
def update_template(template_id: str, data: dict) -> Optional[dict]:
    ...
    try:
        ...
        for field in ('name', 'channel', 'event_types', 'task_domains', 'editor_mode',
                      'subject_template', 'html_template', 'text_template', 'enabled',
